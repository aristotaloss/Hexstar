...
public class DockNode extends VBox implements EventHandler<MouseEvent> {
	...
	public void setFloating(boolean floating, Point2D translation) {
		if (floating && !this.isFloating()) {
			...
			// Don't break if we had no parent
			if (floatScene == null) floatScene = new Point2D(0, 0);
			if (floatScreen == null) floatScreen = new Point2D(0, 0);
			...
			System.out.println(floatScreen);
			if (this.isDecorated()) {
			...
			System.out.println(stage + ", " + stagePosition + "," + insetsDelta);
